...
```{python hidden=TRUE}
major_conflicts[major_conflicts.actor == 'US']
```
...
```{python hidden=TRUE}
geo_deaths = ged_ucdp[(ged_ucdp.best > 0) & (ged_ucdp.best < 20000)][['year','date_start','country','best']].groupby(['year','date_start','country']).sum().reset_index()
...
```{python hidden=TRUE}
tot_per_year = geo_deaths.groupby('date').deaths.sum().reset_index()#.sort_values(by='date', axis=1, ascending=False)

tot_per_year.sort_values('date',inplace=True)

```

```{python hidden=TRUE}
xs = tot_per_year.date.to_list()
ys = tot_per_year.deaths.to_list()
plt.plot(xs,ys)
```
